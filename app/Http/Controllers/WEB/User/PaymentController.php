...
class PaymentController extends Controller
{
    ...
    {
        $client = new Client(config('services.eway.api_key'), config('services.eway.password'), config('services.eway.environment'));

        $response = $client->createTransaction(ApiMethod::DIRECT)
            ->setTransactionType(TransactionType::PURCHASE)
            ->setPaymentDetails($request->input('amount'), 'AUD')
            ->setCardDetails(
                $request->input('card_number'),
                $request->input('card_expiry_month'),
                $request->input('card_expiry_year'),
                $request->input('card_cvn')
            )
            ->setCustomerDetails(
                $request->input('customer_first_name'),
        ...
        if ($response->isTransactionSuccessful()) {
            ...
            // Payment failed
            return redirect()->route('payment.failed')->with('error', 'Payment failed: ' . $response->getMessage());
        }
    }
    ...
    public function orderStore($user, $calculate_amount, $payment_method, $transaction_id, $payment_status, $cash_on_delivery, $address_id){
        ...
        if($user->mobile == 0 || $user->mobile == NULL)
        ...
        if($find_address && $find_address->delivery_area_id !=NULL){
            ...
            $orderAddress = new OrderAddress();
            $orderAddress->order_id = $order->id;
            $orderAddress->name = $user->name;
            $orderAddress->email = $user->email;
            $orderAddress->phone = $order->mobile;
            $orderAddress->address = "I will Pick Up";
            $orderAddress->longitude = 0.0;
            $orderAddress->latitude = 0.0;
            $orderAddress->save();
        }
